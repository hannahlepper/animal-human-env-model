...
@time dat_Bd = pmap(x -> model_run(get_params(1, x, 2000000, 
...
@time dat_H = pmap(x -> model_run(get_params(2, x, 2000000, 
    pf, p_uncertainty, bEH_experiments_unbounded, LA_experiments), unboundeds), 1:13)
@save "/mnt/d/results_workspace_revised_H_1.jld2" dat_H
dat_H = Nothing

@time dat_E = pmap(x -> model_run(get_params(3, x, 2000000, 
    pf, p_uncertainty, bEH_experiments_unbounded, LA_experiments), unboundeds), 1:13)
